...
- **Camera passthrough & recording** — an optional live camera background behind the script, with one-click recording and a "reveal in Finder" shortcut once a take finishes
...
**Option 1 — download the app:** grab the latest DMG from [Releases](https://github.com/tilakp/mac-prompt/releases/tag/latest-release), open it, and drag Prompt into Applications. Since it isn't notarized/signed with a paid Apple Developer account, the first launch needs a right-click → Open (or System Settings → Privacy & Security → "Open Anyway") to get past Gatekeeper.
...
Either way, the first time you open Prompter mode with camera passthrough or voice tracking enabled, macOS will ask for Camera, Microphone, and Speech Recognition permission. Recordings are saved into the app's own sandboxed storage (`~/Library/Containers/com.patelt.mac-prompt/Data/Documents/Recordings`) — use the folder icon that appears next to the record button once a take finishes to jump straight to it in Finder.
